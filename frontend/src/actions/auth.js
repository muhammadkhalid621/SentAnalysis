...
export const signup = (data) => async (dispatch) => {
  ...
  try {
    ...
    alert(
      "Your account has been created. An activation email has been send it to your Email Address. Please Activate your account."
    );
  } catch (error) {
...
export const reset_password = (email) => async (dispatch) => {
  ...
  try {
    ...
    alert(
      "A password reset link has been sent it your Email Address from where you can change your password."
    );
  } catch (error) {
    dispatch({
    ...
    const a = JSON.stringify(error.response.data, null, 2);
    const user_email = JSON.parse(a)["email"];
    if (user_email) alert(JSON.parse(a)["email"][0]);
  }
};
...
export const reset_password_confirm =
  (uid, token, new_password, re_new_password) => async (dispatch) => {
    ...
    try {
      ...
      const a = JSON.stringify(error.response.data, null, 2);
      const user_pass = JSON.parse(a)["new_password"];
      const user_re_pass = JSON.parse(a)["re_new_password"];
      if (user_pass) alert(JSON.parse(a)["new_password"][0]);
      else if (user_re_pass) alert(JSON.parse(a)["re_new_password"][0]);
    }
  };
...
export const get_emails = () => async (dispatch) => {
  if (localStorage.getItem("access")) {
    const config = {
      headers: {
        "Content-Type": "application/json",
    ...
    try {
      const res = await axios.get(`/accounts/getEmails/`, config);
      console.log(res.data);

      dispatch({
        type: EMAIL_UPLOADED_SUCCESS,
        payload: res.data,
      });
    } catch (error) {
      const b = JSON.stringify(error.response.data, null, 2);
      dispatch({
        type: EMAIL_UPLOADED_FAIL,
        payload: JSON.parse(b),
      });
    }
  }
...
export const change_password = (old_password, new_password) => async (dispatch) => {
  if (localStorage.getItem("access")) {
    ...
    try {
      const res = await axios.put(`/accounts/change-pass/`, body, config);
      console.log(res.data);
