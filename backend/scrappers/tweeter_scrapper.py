...
# def tweeter_scrapper(text ,numberOfTweets):
#     scraped_tweets = sntwitter.TwitterSearchScraper(f'{text}').get_items()
#     sliced_scraped_tweets = itertools.islice(scraped_tweets, numberOfTweets)
#     tweets_df = pd.DataFrame(sliced_scraped_tweets)
#     tweets_all_df = pd.concat([ tweets_df ], axis=0)
#     tweets_all_df.to_csv("{0}.csv".format(text), index=False)
#     return tweets_df
