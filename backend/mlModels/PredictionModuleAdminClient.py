...
def sarcasmPredictions(file_name, df, clean_tweets, tf, keras, Tokenizer, pad_sequences, pd, np, model_from_json, code):

    json = open(os.path.join(str(settings.MEDIA_ROOT) +
                '/model_files', 'SSK_SarcasmModel.json'), 'r').read()
    modelSarcasm = model_from_json(json)

    # load weights into new model
    modelSarcasm.load_weights(os.path.join(
    ...
    print(file_name)
    Sarcasmdf.to_csv(os.path.join(str(settings.MEDIA_ROOT) + '/predict_files/twitter',
                     file_name.split('.')[0] + '_Sarcasm' + '_' + code + '.csv'), index=False)
    Sentimentdf = df[df['Sarcasm'] == 0]
    return Sentimentdf


def sentimentPredictions(file_name, Sentimentdf, tf, keras, Tokenizer, pad_sequences, pd, np, model_from_json, code):
    ...
    Sentimentdf.to_csv(os.path.join(str(settings.MEDIA_ROOT) + '/predict_files/twitter',
                       file_name.split('.')[0] + '_Sentiments' + '_' + code + '.csv'), index=False)
...
def Clean_tweets(test_data, re, string, stopwords, PorterStemmer):
    cleaned_tweets = []
    for i in test_data:
        cleaned_tweets.append(process_tweet(
            i, re, string, stopwords, PorterStemmer))
    return cleaned_tweets


def sskPredictor(data, file_name, code):
    ...
    clean_tweets = Clean_tweets(
        test_data, re, string, stopwords, PorterStemmer)
    Sentimentdf = sarcasmPredictions(
        file_name, df, clean_tweets, tf, keras, Tokenizer, pad_sequences, pd, np, model_from_json, code)
    sentimentPredictions(file_name, Sentimentdf, tf, keras,
                         Tokenizer, pad_sequences, pd, np, model_from_json, code)
