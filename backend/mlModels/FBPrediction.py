...
def sskFBPredictor(data, file_name, code):
  ...
  # df = df[['user_id','user_url','post_url','link','text','Polarity','Sentiment','likes','comments','shares']]
  # df.to_csv(data[:(len(data)) - 4] + 'PostSentiments.csv', index=False)
  # return df
  PosPosts = df[df['Sentiment'] == 1]
  NegPosts = df[df['Sentiment'] == 0]
  
  PosPosts.to_csv(os.path.join(str(settings.MEDIA_ROOT) + '/predict_files/facebook',
                       file_name.split('.')[0] + '_' + code + '_PosPosts.csv'),index=False)
  NegPosts.to_csv(os.path.join(str(settings.MEDIA_ROOT) + '/predict_files/facebook',
                       file_name.split('.')[0] + '_' + code + '_NegPosts.csv'),index=False)
